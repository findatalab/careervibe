...
def inject_user():
    data = {'user_id': None, 'user_nickname': None, 'user_avatar': None}
    if 'user_id' in session:
        db = get_db()
...
def edit_question(question_id):
    ...
    if request.method == 'POST':
        ...
        if len(tag_names) > 7:
            flash('Нельзя добавить больше 7 тегов.', 'error')
            return redirect(url_for('edit_question', question_id=question_id))
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        if not nickname:
            flash('Никнейм обязателен.', 'error')
    ...
    if request.method == 'POST':
        ...
        if action == 'login':
            ...
            if user and check_password_hash(user['password_hash'], pwd):
                ...
                return redirect(url_for('index'))
            flash('Неверный email/ник или пароль.', 'error')
            ...
            if len(pwd) < 4:
                ...
                return redirect(url_for('login'))
            if len(nickname) < 3:
                flash('Никнейм не менее 3 символов.', 'error')
                return redirect(url_for('login'))
